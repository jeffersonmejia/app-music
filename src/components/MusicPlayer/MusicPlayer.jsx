...
export const MusicPlayer = ({ artist, audioSrc, title }) => {
  ...
  const savePlayerState = useCallback((state) => {
  ...
  useEffect(() => {
    ...
    audio.currentTime = Number.isFinite(savedPlayer.currentTime) ? savedPlayer.currentTime : 0;
  ...
  }, [fadeVolumeTo, savePlayerState, savedPlayer.currentTime, savedPlayer.isPlaying]);

  return (
    <section className={`music-player${pathname === "/" ? " music-player--home" : ""}`} aria-label={`${title} player`}>
      ...
      <div className="music-player__controls">
        ...
        <div className="music-player__actions" aria-label="Playback controls">
          <button className="player-control" type="button" disabled aria-label="Previous blocked">
          ...
          <button className="player-control" type="button" disabled aria-label="Next blocked">
